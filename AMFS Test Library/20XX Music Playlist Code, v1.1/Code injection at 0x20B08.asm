...
# May be useful: 80225160 - StartMelee_LoadMusic, just before call to Music_HandleLoading
...
backupRegisters:		# Backup registers
  ...
  stw r20, 4(r1)		# Store 0 (word) to memory address r1+4		# 	Hex Tracks	  01=Menu 1, 27=Yoshi's Story, 1C=Fountain of Dreams
																					# 26=Pokemon Stadium, 21=Dream Land 64, 2E=Battlefield
																					# 2B=Final Destination, 0B=Green Greens, 03=Trophy Stage,
																					# 1D=Hyrule Temple, 01 (originally "testnz.hps")

checkForHexTrackFlag:	# Check if the song ID has a flag to use a hex track. If it does, check whether there's a global playlist set up (use that if it is)
  mr r4, r3				# Need to copy it before making modifications
  srwi r4, r4, 16		# Get the flag by itself (bit shift right 16). If set, the flag should be at 0x10000
  cmpwi r4, 0
  beq+ loopStart		# Continue normal code execution if there's no flag
  lis r4, 0x8040
  lwz r4, -20844(r4)	# Load Debug Menu "Global Playlist" flag (byte at 803FAE94|0x3F7E94) to r4
  cmpwi r4, 0
  bne loopStart			# Continue normal code execution if using a global playlist (when flag is set)
  li r16, 0x62
  stw r16, 32(r1)		# Set game's original song ID (previously in r3) to 0x62 (testnz)
  lis r15, 0x803B
  ori r15, r15, 0xC308	# Set a destination for the new ASCII hex track name to the location originally for testnz
  li r20, 0xA			# Set the loop counter such that the loop will exit immediately
  slwi r3, r3, 16		# Mask out the hex flag from the music ID
  srwi r16, r3, 16		# Mask out the hex flag from the music ID (part 2). Moving song ID to r16 to prepare for ASCII conversion
  b convertFirstDigit

loopStart:				# Start iterating over pointers in above table
  li r5, 0x0
  cmpwi r20, 0xA
...
loopFinish:
  ...
  cmpwi r3, 0x62
  beq- restoreRegisters	# Exit this function
  lis r16, 0x8040
  lwz r16, -20844(r16)	# Load Debug Menu "Global Playlist" flag (byte at 803FAE94|0x3F7E94) to r16
  cmpwi r16, 0x0
  beq- restoreRegisters	# Exit this function if not using global playlist (flag is 0)
...
songIsMainMenu:			# Current song should be Main Menu or Main Menu Alt; Check if the main menu uses global playlist as well
  ...
  cmpwi r16, 0x0
  beq- restoreRegisters	# Exit this function if not using global playlist for menus (flag is 0)
  li r16, 0x62
...
analyzePlaylist:		# Loads variables needed for playlist traversal and determining a new song ID. Uses Playlist Type to determine how to move forward
  li r3, 0x0
  lis r4, 0x8000
  lwz r5, 13040(r4)		# Load custom song count at 800032F0|0x2F0 to r5. This value here is updated on boot
  lis r4, 0x8038
...
storeMusicID:
  ...
  sth r18, 0(r15)		# Store both ASCII digits to address in r15		(address of this instruction during testing: )
...
END:
  b 0					# Branch back (exit injection), to 80023F2C
